...
class hsv_threshold(Node):
    def __init__(self):
        ...
        self.bgr_pub = self.create_publisher(CompressedImage, "/bgr/compressed", 10)
        self.hsv_pub = self.create_publisher(CompressedImage, "/hsv/compressed", 10)
        self.range_pub = self.create_publisher(CompressedImage, "/range/compressed", 10) 
        
        self.mask_pub = self.create_publisher(CompressedImage, "/mask/compressed", 10)
        self.final_mask_pub = self.create_publisher(CompressedImage, "/finalmask/compressed", 10)
    ...
    def image_feed_callback(self, msg):
        ...
        dilated = cv2.dilate(eroded, kernel1, iterations=5)
