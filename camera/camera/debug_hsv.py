...
class hsv_threshold(Node):
    def __init__(self):
        ...
        self.bgr_pub = self.create_publisher(
            CompressedImage, "debug/bgr/compressed", 10
        )
        self.hsv_pub = self.create_publisher(
            CompressedImage, "debug/hsv/compressed", 10
        )
        self.range_pub = self.create_publisher(
            CompressedImage, "debug/range/compressed", 10
        )  # not sure whats this for

        self.mask_pub = self.create_publisher(
            CompressedImage, "debug/mask/compressed", 10
        )
        self.final_mask_pub = self.create_publisher(
            CompressedImage, "debug/finalmask/compressed", 10
        )
...
def create_gui(node):
    ...
    cv2.createTrackbar(
        "Lower Hue", "HSV Bounds", SETTINGS["LOWER_HUE"], 255, update_bounds
    )
    cv2.createTrackbar(
        "Upper Hue", "HSV Bounds", SETTINGS["UPPER_HUE"], 255, update_bounds
    )
    cv2.createTrackbar(
        "Lower Saturation",
        "HSV Bounds",
    ...
    cv2.createTrackbar(
        "Lower Value", "HSV Bounds", SETTINGS["LOWER_VALUE"], 255, update_bounds
    )
    cv2.createTrackbar(
        "Upper Value", "HSV Bounds", SETTINGS["UPPER_VALUE"], 255, update_bounds
    )
    while True:
        key = cv2.waitKey(1)
        if key == ord("s"):
...
def update_bounds(val):
    try:
        ...
        SETTINGS["LOWER_SATURATION"] = cv2.getTrackbarPos(
            "Lower Saturation", "HSV Bounds"
        )
        SETTINGS["UPPER_SATURATION"] = cv2.getTrackbarPos(
            "Upper Saturation", "HSV Bounds"
        )
