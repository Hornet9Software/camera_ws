...
import rclpy
from controls_core.params import front_weights_file_path
...
model = YOLO(front_weights_file_path)
...
class GateDetectorNode(Node):
    def __init__(self):
        ...
        self.yolo_pub = self.create_publisher(
            CompressedImage, f"{namespace}/yolo/compressed", 10
        )
        self.bbox_pub = self.create_publisher(
            Float32MultiArray, f"{namespace}/yolo/box", 10
        )
