  ...
  theora_image_transport__msg__Packet__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    theora_image_transport__msg__Packet * data =
      (theora_image_transport__msg__Packet *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!theora_image_transport__msg__Packet__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          theora_image_transport__msg__Packet__fini(&output->data[i]);
        }
        return false;
      }
    }
